...
public class LiveDisplayController {
    ...
    void systemReady() {
        ...
        mUseOutdoorMode =
                mHardware.isSupported(MKHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT);
        mOutdoorModeIsSelfManaged = mUseOutdoorMode ?
                mHardware.isSunlightEnhancementSelfManaged() : false;

        mUseLowPower =
                mHardware.isSupported(MKHardwareManager.FEATURE_ADAPTIVE_BACKLIGHT);
        if (mUseLowPower) {
            mLowPower = mHardware.get(MKHardwareManager.FEATURE_ADAPTIVE_BACKLIGHT);
        }

        mUseColorEnhancement =
                mHardware.isSupported(MKHardwareManager.FEATURE_COLOR_ENHANCEMENT);
        if (mUseColorEnhancement) {
            mColorEnhancement =
                mHardware.get(MKHardwareManager.FEATURE_COLOR_ENHANCEMENT);
        }
    ...
    private void updateGamma() {
        ...
        for (int i = 0; i < mHardware.getNumGammaControls(); i++) {
            List<String> gammaValue = MKSettings.Secure.getDelimitedStringAsList(
                    contentResolver, DELIMITER,
                    MKSettings.Secure.DISPLAY_GAMMA_CALIBRATION_PREFIX + i);
            if (gammaValue != null) {
    ...
    private final class SettingsObserver extends UserContentObserver {
        ...
        public void register(boolean register) {
            ...
            if (register) {
                ...
                if (mHardware.isSupported(MKHardwareManager.FEATURE_DISPLAY_GAMMA_CALIBRATION)) {
                    for (int i = 0; i < mHardware.getNumGammaControls(); i++) {
                        Uri gammaUri = MKSettings.Secure.getUriFor(
                                MKSettings.Secure.DISPLAY_GAMMA_CALIBRATION_PREFIX + i);
    ...
    private synchronized void updateLowPowerMode() {
        ...
        mHardware.set(MKHardwareManager.FEATURE_ADAPTIVE_BACKLIGHT, enabled);
        mLowPower = enabled;
    }
