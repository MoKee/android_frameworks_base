...
public class SettingsProvider extends ContentProvider {
        ...
        switch (args.table) {
            case TABLE_GLOBAL: {
                if (args.name != null) {
                    ...
                    if (MKSettings.Global.shouldInterceptSystemProvider(args.name)) {
            ...
            case TABLE_SECURE: {
                ...
                if (args.name != null) {
                    ...
                    if (MKSettings.Secure.shouldInterceptSystemProvider(args.name)) {
                        return forwardedQuery(setting, normalizedProjection, args.table);
                    }
                    return packageSettingForQuery(setting, normalizedProjection);
                } else {
            ...
            case TABLE_SYSTEM: {
                ...
                if (args.name != null) {
                    ...
                    if (MKSettings.System.shouldInterceptSystemProvider(args.name)) {
                        return forwardedQuery(setting, normalizedProjection, args.table);
                    }
                    return packageSettingForQuery(setting, normalizedProjection);
                } else {
    ...
    private BaseSetting getGlobalSetting(String name) {
        ...
        // If MKSettingsProvider owns this key, override the value
        if (MKSettings.Global.shouldInterceptSystemProvider(name)) {
            final ContentResolver cr = getContext().getContentResolver();
            final String value = MKSettings.Global.
    ...
    private boolean mutateGlobalSetting(String name, String value, int requestingUserId,
        ...
        // If MKSettingsProvider wants to own this key, let it.
        if (MKSettings.Global.shouldInterceptSystemProvider(name)) {
            switch (operation) {
                case MUTATION_OPERATION_INSERT:
                case MUTATION_OPERATION_UPDATE:
                    final ContentResolver cr = getContext().getContentResolver();
                    return MKSettings.Global.putStringForUser(cr, name, value,
                            UserHandle.USER_OWNER);
    ...
    private BaseSetting getSecureSetting(String name, int requestingUserId) {
        ...
        // If MKSettingsProvider owns this key, override the value
        if (MKSettings.Secure.shouldInterceptSystemProvider(name)) {
            final ContentResolver cr = getContext().getContentResolver();
            final String value = MKSettings.Secure.getStringForUser(cr, name, owningUserId);
    ...
    private boolean mutateSecureSetting(String name, String value, int requestingUserId,
        ...
        if (MKSettings.Secure.shouldInterceptSystemProvider(name)) {
            switch (operation) {
                ...
                case MUTATION_OPERATION_UPDATE:
                    final ContentResolver cr = getContext().getContentResolver();
                    return MKSettings.Secure.putStringForUser(cr, name, value, owningUserId);
                case MUTATION_OPERATION_DELETE:
                    // unsupported
                    return false;
            }
    ...
    private BaseSetting getSystemSetting(String name, int requestingUserId) {
        ...
        // If MKSettingsProvider owns this key, override the value
        if (MKSettings.System.shouldInterceptSystemProvider(name)) {
            final ContentResolver cr = getContext().getContentResolver();
            final String value = MKSettings.System.getStringForUser(cr, name, owningUserId);
    ...
    private boolean mutateSystemSetting(String name, String value, int runAsUserId,
        ...
        if (MKSettings.System.shouldInterceptSystemProvider(name)) {
            switch (operation) {
                ...
                case MUTATION_OPERATION_UPDATE:
                    final ContentResolver cr = getContext().getContentResolver();
                    return MKSettings.System.putStringForUser(cr, name, value, owningUserId);
                case MUTATION_OPERATION_DELETE:
                    // unsupported
                    return false;
            }
    ...
    private void enforceRestrictedSystemSettingsMutationForCallingPackage(int operation,
        ...
        if (MKSettings.System.shouldInterceptSystemProvider(name)) {
            // this setting will be forwarded to MKSettingsProvider
            return;
        }
    ...
    private MatrixCursor forwardedQuery(BaseSetting setting, String[] projection, String table) {
        ...
        switch (table) {
            case TABLE_SYSTEM:
                setting.update(MKSettings.System.getString(getContext().getContentResolver(),
                                setting.getName()), setting.getPackageName());
                break;
            case TABLE_GLOBAL:
                setting.update(MKSettings.Global.getString(getContext().getContentResolver(),
                        setting.getName()), setting.getPackageName());
                break;
            case TABLE_SECURE:
                setting.update(MKSettings.Secure.getString(getContext().getContentResolver(),
                        setting.getName()), setting.getPackageName());
                break;
    ...
    final class SettingsRegistry {
        ...
        private final class UpgradeController {
            ...
            public void upgradeIfNeededLocked() {
                ...
                //
                // MOKEE OPEN SOURCE
                // We moved our settings out to another settings provider (MKSettingsProvider)
                ...
                if ((oldVersion == newVersion || oldVersion == MK_SETTINGS_DB_VERSION)) {
                    if (oldVersion == MK_SETTINGS_DB_VERSION) {
