...
public class NavigationRingHelpers {
    ...
    public static String[] getTargetActions(Context context) {
        ...
        for (int i = 0; i < MAX_ACTIONS; i++) {
            result[i] = MKSettings.Secure.getStringForUser(cr,
                    MKSettings.Secure.NAVIGATION_RING_TARGETS[i], UserHandle.USER_CURRENT);
            if (result[i] != null) {
                isDefault = false;
            }
        }
    ...
    public static void resetActionsToDefaults(Context context) {
        final ContentResolver cr = context.getContentResolver();
        MKSettings.Secure.putStringForUser(cr, MKSettings.Secure.NAVIGATION_RING_TARGETS[0], null,
                UserHandle.USER_CURRENT);
        MKSettings.Secure.putStringForUser(cr, MKSettings.Secure.NAVIGATION_RING_TARGETS[1],
                ACTION_ASSIST, UserHandle.USER_CURRENT);
        MKSettings.Secure.putStringForUser(cr, MKSettings.Secure.NAVIGATION_RING_TARGETS[2], null,
                UserHandle.USER_CURRENT);
    }
