...
public class DataUsageDetailView extends LinearLayout {
    ...
    public void bind(DataUsageController.DataUsageInfo info) {
        ...
        if (activeSimCount > 1) {
            ...
            for (int slotIndex = 0; slotIndex < activeSimCount; slotIndex++) {
                String carrierName = getCarrierName(slotIndex);
            ...
            int defaultDataSlotIndex = SubscriptionManager.getSlotIndex(defaultDataSubId);
            android.util.Log.d("hcz", "defaultDataSlotIndex: " + defaultDataSlotIndex);
            if(-1 != defaultDataSlotIndex){
                RadioButton radioButton = (RadioButton) mRadioGroup.getChildAt(defaultDataSlotIndex);
                radioButton.setChecked(true);
            }

            // set checkListener
            mRadioGroup.setOnCheckedChangeListener((group, checkedId) -> {
                for (int slotIndex = 0; slotIndex < getActiveSimCount(); slotIndex++) {
                    if ((mRadioGroup.getChildAt(slotIndex).getId() == checkedId)) {
    ...
    private String getCarrierName(int slotIndex) {
        SubscriptionManager sm = SubscriptionManager.from(mContext);
        List<SubscriptionInfo> subs = sm.getActiveSubscriptionInfoList();
        CharSequence carrierName = subs.get(slotIndex).getCarrierName();
