...
package android.mokee.internal;
 ...
 * @hide
 */
public class BootMsgDialog extends Dialog {
    ...
    private TextView mBootMsg;
    private TextView mBootMsgDetail;
    ...
    public static BootMsgDialog create(Context context) {
        return create(context,  WindowManager.LayoutParams.TYPE_BOOT_PROGRESS);
    }

    public static BootMsgDialog create(Context context, int windowType) {
        ...
        return new BootMsgDialog(context, theme, windowType);
    }

    private BootMsgDialog(Context context, int themeResId, int windowType) {
        super(context, themeResId);
        mContext = context;
        mPackageManager = mContext.getPackageManager();

        final LayoutInflater inflater = LayoutInflater.from(mContext);
        final View bootMsgLayout = inflater.inflate(R.layout.boot_layout, null, false);
        mBootMsg = (TextView) bootMsgLayout.findViewById(R.id.boot_message);
        mBootMsgDetail = (TextView) bootMsgLayout.findViewById(R.id.boot_message_detail);
        mBootIcon = (ImageView) bootMsgLayout.findViewById(R.id.boot_icon);
        mBootProgress = (ProgressBar) bootMsgLayout.findViewById(R.id.boot_progress);
        mBootProgress.setProgressTintList(ColorStateList.valueOf(Color.WHITE));
        ...
        bootMsgLayout.post(new Runnable() {
            @Override public void run() {
                mBootIcon.setImageDrawable(null);

                // start the marquee
                mBootMsg.setSelected(true);
                mBootMsgDetail.setSelected(true);
            }
        });
    }
    ...
    public void setProgress(final ApplicationInfo info, final int current, final int total) {
        ...
        if (info == null) {
            ...
            mBootProgress.setProgress(current);
            // just make it look pretty :P
            if ((current + 1) <= total) {
                mBootProgress.setSecondaryProgress(current + 1);
            }
        }
        ...
        if (mWasApk != isApk) {
            mWasApk = isApk;
            if (isApk) {
                mBootMsgDetail.setVisibility(View.VISIBLE);
                mBootProgress.setVisibility(View.VISIBLE);
            } else {
                mBootMsgDetail.setVisibility(View.GONE);
                mBootProgress.setVisibility(View.INVISIBLE);
            }
        }

        // if we are processing an apk, load its icon and set the message details
        if (isApk) {
            mBootIcon.setImageDrawable(info.loadIcon(mPackageManager));
            mBootMsgDetail.setText(String.format("(%s)", info.packageName));
        } else {
            mBootIcon.setImageDrawable(null);
        }

        mBootMsg.setText(msg);
    }
